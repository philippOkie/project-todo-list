...
function removeNoteFromLibrary(noteId, myFoldersLocalStorage, i) {
  myFoldersLocalStorage[i].notes = myFoldersLocalStorage[i].notes.filter(
    (note) => note.noteId !== noteId
  );
}

function removeFolderFromLibrary(folderId) {
  myFolders = myFolders.filter(
    (folderToArr) => folderToArr.folderId !== folderId
  );
}
...
export function showFolders() {
  ...
  for (let i = 0; i < myFoldersLocalStorage.length; i++) {
    ...
    folderName.addEventListener("click", () => {
      ...
      myFolders = myFolders.filter(
        (myFoldersLocalStorage) => myFoldersLocalStorage.folderId !== folderId
      );
        ...
        if (titleNote !== "") {
          ...
          deleteNoteBtn.addEventListener("click", () => {
            if (newNote && newNote.parentNode) {
              ...
              removeNoteFromLibrary(note.noteId, myFoldersLocalStorage, i);
